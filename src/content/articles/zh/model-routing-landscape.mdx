...
关键洞察是：这四个维度不可能同时最优，但可以**按 query 分别优化**。RouteLLM（Ong et al., 2024）在实验中证明，通过训练路由器在 GPT-4 和较小模型之间智能切换，可以在几乎不损失质量的前提下，将 API 调用成本降低 2 倍以上。FrugalGPT（Chen et al., 2023）更是展示了通过级联策略，在特定基准任务上实现高达 98% 的成本削减（其他任务为 50-73%）。
