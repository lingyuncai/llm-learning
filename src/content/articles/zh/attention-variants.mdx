...
softmax 之所以阻止结合律，是因为它做了两件事：(1) exp() 保证非负，(2) **逐行归一化**（除以行和）。归一化耦合了同一行的所有元素 — 计算 $\text{softmax}(q_i^T k_1)$ 需要知道 $q_i^T k_2, q_i^T k_3, ..., q_i^T k_n$。你不能独立算一个元素，必须先算完整行的 $QK^T$（$n \times n$）。

$\phi$ 的策略：**用独立的逐元素变换替代耦合的归一化**。将 softmax 核 $\text{sim}(q, k) = \exp(q^T k) / Z$ 替换为核函数分解：

$$\text{sim}(q, k) = \phi(q)^T \phi(k)$$

$\phi$ 独立作用于每个 $q$ 和 $k$（不需要知道其他 key 的值），而内积天然满足结合律。展开第 $i$ 个输出（含归一化）：

$$o_i = \frac{\phi(q_i)^T \overbrace{\sum_j \phi(k_j) v_j^T}^{d \times d,\ \text{与 } i \text{ 无关}}}{\phi(q_i)^T \underbrace{\sum_j \phi(k_j)}_{d \times 1,\ \text{与 } i \text{ 无关}}}$$

分子和分母中的求和都**与查询位置 $i$ 无关**，可以预先算好一次（$O(nd)$），然后每个查询只做 $O(d)$ 的向量乘法。

**φ 的选择**需要满足两个条件：(1) **输出非负** — 确保注意力权重非负，(2) **逐元素独立** — 不能像 softmax 那样耦合同行其他元素。常见选择包括 $\phi(x) = \text{elu}(x) + 1$（Katharopoulos 2020 原始选择）和 $\phi(x) = \text{ReLU}(x)$。理论上，Random Fourier Features 可以近似 softmax 的 exp 核，但计算较贵。

> **φ 效果不如 softmax 的根本原因**：softmax 的 exp + 归一化天然产生**尖锐、稀疏的注意力分布**（大值指数放大，小值压到接近零），让模型能集中关注最相关的少数 token。简单的 φ 没有这种"赢者通吃"效果，所有 key 的贡献差异不大。这就是后续工作（RetNet/DeltaNet/GDN）加衰减、delta rule、门控的本质动机 — 弥补 φ 无法替代 softmax 选择性的缺陷。
