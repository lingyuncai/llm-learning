...
这条看似简单的规则，彻底改变了编译器的设计方式。SSA 的概念由 IBM 研究员在 1980 年代逐步发展（Rosen、Wegman 和 Zadeck 在 1988 年引入了 $\phi$ 函数）。Cytron 等人在 1991 年的经典论文 "Efficiently Computing Static Single Assignment Form and the Control Dependence Graph" 中提出了高效构造 SSA 的算法（基于 dominance frontier），使 SSA 成为现代编译器的标准。此后，几乎所有现代编译器都采用 SSA 作为标准 IR 形式——LLVM IR、GCC GIMPLE、Java HotSpot 的 Sea of Nodes、V8 的 TurboFan（后被 Turboshaft 替代），无一例外。
