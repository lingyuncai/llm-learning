...
> **源码级深入**：`n_gpu_layers` 层分配算法（`i_gpu_start` 计算 + 多 GPU `tensor_split`）详见 [模型加载：从文件到设备](/zh/llama-cpp-model-loading)；五遍扫描 backend 调度算法详见 [Backend 调度、Op Fusion 与内存分配](/zh/llama-cpp-scheduling-memory)。
