...
Council Mode (2024) 是多模型协作的一种经典架构。它的工作流程分为两个阶段：
...
**HieraMAS** (Hierarchical Multi-Agent System) 在每个决策节点 (decision node) 上运行一个 mini-council，每层的输出成为下一层的输入。这样可以处理需要多步推理 (multi-step reasoning) 的复杂任务。
...
- 从 1 个模型增加到 3 个模型：质量提升约 **+6%**，成本增加 **3×**。
- 从 3 个模型增加到 10 个模型：质量提升仅 **+2%**，成本再增加 **3.3×**。

边际收益 (marginal benefit) 快速下降，而边际成本 (marginal cost) 线性增长。对于大多数应用，**2-3 个模型**是性价比的甜蜜点 (sweet spot)。
