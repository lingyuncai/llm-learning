...
We've explained how KV Cache works, but a natural question arises: Attention involves Q, K, and V — **why only cache K and V, not Q?** Furthermore, **why not cache the $QK^T$ attention scores directly?**

### Why Not Cache Q?

The key insight is that **Q is "fire-and-forget," while K and V are "persistently needed."**

At decode step $t$, the new token's query $q_t$ has exactly one purpose: compute dot products with all cached Keys to produce attention scores. Once the current step's attention is complete, $q_t$ is never used by any subsequent step.

In contrast, look at how K and V are used:

- **$k_t$ is needed by all future steps**: the new query at steps $t+1, t+2, ..., N$ all need to compute dot products with $k_t$
- **$v_t$ is needed by all future steps**: every future step's attention weighted sum must include $v_t$
...
### Why Not Cache $QK^T$ Attention Scores?

Another intuition might be: since step $t$ already computed the dot product scores between $q_t$ and $k_1, ..., k_t$, can we store these scores for later use?

The answer is **no**, because the query is different at every step:

- Step $t$: $\text{scores} = [q_t \cdot k_1,\ q_t \cdot k_2,\ ...,\ q_t \cdot k_t]$
- Step $t+1$: $\text{scores} = [q_{t+1} \cdot k_1,\ q_{t+1} \cdot k_2,\ ...,\ q_{t+1} \cdot k_{t+1}]$

Although $k_1, ..., k_t$ haven't changed, $q_{t+1} \neq q_t$, so **every score must be recomputed with the new query**. The attention score is a bivariate function of Q and K — the K side is constant (hence we cache K), but the Q side is entirely new each step, making the function values (scores) completely non-reusable.
...
In a multi-layer Transformer, the complete data flow at decode step $t$ is:
...
And $h_t^{(\ell-1)}$ itself requires the attention computation at layer $\ell-1$. This means:
...
| **Recurrence** | $k_t^{(\ell)} = f(h_t^{(\ell-1)})$; deeper layers depend on shallower attention outputs |
| **No aftereffect** | Causal mask guarantees: position $i$'s K/V depends only on $1, ..., i$, unaffected by future tokens |
| **Memoization** | Store computed K/V in cache; subsequent steps look up directly |
| **Complexity optimization** | Total linear projections reduced from $O(N^2 d^2)$ to $O(N d^2)$ |
