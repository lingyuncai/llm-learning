...
Council Mode (2024) is a classic architecture for multi-model collaboration. Its workflow consists of two phases:
...
**HieraMAS** (Hierarchical Multi-Agent System) runs a mini-council at each decision node, where each layer's output becomes the next layer's input. This enables handling complex tasks that require multi-step reasoning.
...
- Going from 1 model to 3 models: quality improves by about **+6%**, cost increases **3x**.
- Going from 3 models to 10 models: quality improves by only **+2%**, cost increases another **3.3x**.

Marginal benefit drops rapidly while marginal cost grows linearly. For most applications, **2-3 models** is the cost-effectiveness sweet spot.
