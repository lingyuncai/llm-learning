...
1. **Hardware access patterns**: Different hardware has varying optimization levels for contiguous memory access. GPU Tensor Cores require the K dimension of matrix multiplication to be contiguous; NHWC naturally satisfies this when K=C
2. **Cache utilization**: Convolution operations slide over spatial dimensions; NHWC keeps adjacent spatial positions in cache
...
This mixed layout strategy is about 15-20% faster than pure NCHW or pure NHWC.
...
On A100 GPU, for typical GPT-3 attention (`B=1, H=96, S=2048, D=128`):
