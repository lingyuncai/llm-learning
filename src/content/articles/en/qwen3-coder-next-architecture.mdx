...
First, the old state undergoes per-element exponential decay ($\alpha_t$ is input-dependent, giving each state element its own decay rate). Then the delta rule update is applied on the decayed state — retrieving the old value ($\tilde{H}_{t-1} k_t$), computing the correction, and writing it back. This combines flexible gating with precise associative memory.

### 2.2 Complete Data Flow and Input Processing

Section 2.1 presented the mathematical formulas for Gated DeltaNet, but there's a gap between formulas and the actual model. Which GGUF tensor corresponds to which part of the formula? What transformations does data undergo from input to output? Below is the complete forward pass for a GatedDeltaNet layer, followed by detailed explanations.
...
Note the **asymmetric head structure**: K heads = 16, V heads = 32 (2:1 ratio), head_dim = 128. Having 2x more V-heads means the state matrix $H$ is wider in the V direction ($128 \times 4096$, where $4096 = 128 \times 32$), increasing memory capacity.
...
After Conv1d splits out Q, K, V, several additional processing steps are applied:
