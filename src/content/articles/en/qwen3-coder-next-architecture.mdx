...
The difference from DeltaNet: before updating, the old state undergoes per-element exponential decay ($\alpha_t$ is input-dependent, giving each state element its own decay rate). Then the same delta rule is applied on the decayed state $\tilde{H}_{t-1}$:

1. **Retrieve**: $\tilde{H}_{t-1} k_t$ — query the old value from the decayed state
2. **Compute delta**: $v_t - \tilde{H}_{t-1} k_t$ — error between new target and old memory
3. **Write correction**: $\beta_t \cdot k_t (\cdots)^\top$ — write the correction back as an outer product
