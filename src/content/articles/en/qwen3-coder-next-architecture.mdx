...
The previous three articles introduced [MoE](/en/articles/mixture-of-experts), [SSM](/en/articles/state-space-models), and [Hybrid architectures](/en/articles/hybrid-architectures) as separate concepts. This article uses a real production model — **Qwen3-Coder-Next 80B** — to show how all three work together in a single architecture.
...
Recall Mamba's state update from the [SSM article](/en/articles/state-space-models):
...
This project → conv1d → recurrence three-stage pipeline is inherited from Mamba — the same pattern introduced in the [SSM article](/en/articles/state-space-models). After Conv1d + SiLU, the output is split into Q, K, V vectors for subsequent normalization and state update.
...
State propagates across chunks, similar to [Mamba-2's SSD algorithm](/en/articles/state-space-models).
...
This is **8:1 GQA** ([Grouped-Query Attention](/en/articles/gqa-mqa)) — 16 Q heads share 2 KV groups.
...
All 48 layers use the same [MoE](/en/articles/mixture-of-experts) FFN structure.
...
A [DeepSeek-style](/en/articles/mixture-of-experts) shared expert that all tokens pass through, with sigmoid gating:
...
- [Mixture of Experts](/en/articles/mixture-of-experts) — MoE routing, load balancing, DeepSeek innovations
- [State Space Models and Mamba](/en/articles/state-space-models) — SSM basics, selective mechanism, SSD duality
- [Hybrid Architectures](/en/articles/hybrid-architectures) — Jamba, Zamba2, Hymba design comparison
- [GQA and MQA](/en/articles/gqa-mqa) — Grouped-Query Attention principles
- [KV Cache](/en/articles/kv-cache) — Caching mechanisms and optimization
