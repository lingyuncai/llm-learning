...
const createMethods = (locale: 'zh' | 'en'): MethodPoint[] => {
  const t = {
    zh: {
      ...
      mfRouter: { name: 'MF Router', detail: 'RouteLLM Matrix Factorization。偏好数据学评分，2x+ 成本降低。' },
      ...
      frugalGpt: { name: 'FrugalGPT', detail: '级联链：便宜模型先试，不行再升级。50-98% 成本降低（因任务而异）。' },
    ...
    en: {
      ...
      mfRouter: { name: 'MF Router', detail: 'RouteLLM Matrix Factorization. Learn scoring from preference data, 2x+ cost reduction.' },
      ...
      frugalGpt: { name: 'FrugalGPT', detail: 'Cascade chain: try cheap model first, upgrade if needed. 50-98% cost reduction (varies by task).' },
  ...
  return [
    ...
    { name: t.mfRouter.name, accuracy: 93, costSaving: 55, category: 'classifier' as const, detail: t.mfRouter.detail },
    { name: t.bertRouter.name, accuracy: 91, costSaving: 50, category: 'classifier' as const, detail: t.bertRouter.detail },
    { name: t.causalLmRouter.name, accuracy: 92, costSaving: 75, category: 'classifier' as const, detail: t.causalLmRouter.detail },
    { name: t.semanticRouter.name, accuracy: 85, costSaving: 70, category: 'classifier' as const, detail: t.semanticRouter.detail },
    { name: t.frugalGpt.name, accuracy: 96, costSaving: 80, category: 'cascade' as const, detail: t.frugalGpt.detail },
    ...
    { name: t.paretoBandit.name, accuracy: 90, costSaving: 50, category: 'online' as const, detail: t.paretoBandit.detail },
