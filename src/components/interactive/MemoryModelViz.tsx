...
const MemoryModelViz: React.FC<{ locale?: 'zh' | 'en' }> = ({ locale = 'zh' }) => {
  const t = {
    zh: {
      title: 'iGPU 统一内存模型：Host / Device / Shared (USM)',
      cpuLabel: 'CPU',
      cpuModel: 'Core i7-1370P',
      cpuCores: '14 cores',
      igpuLabel: 'iGPU',
      igpuModel: 'Xe2-LPG',
      igpuCores: '8 Xe cores',
      systemMemory: '统一系统内存 (LPDDR5x)',
      systemMemoryDesc: 'CPU 与 iGPU 共享物理内存，无独立显存',
      fast: '快速',
      cacheCoherent: 'cache coherent',
      slow: '慢',
      optimized: '优化',
      direct: '直接',
      zeroCopy: '零拷贝！',
      hostDesc: 'Host: CPU 优化，GPU 通过 cache coherence 访问 (适合 CPU 主导计算)',
      deviceDesc: 'Device: GPU 优化，CPU 访问慢 (适合 GPU 密集计算，结果稀疏)',
      sharedDesc: 'Shared (USM): 零拷贝！iGPU 独特优势，CPU/GPU 直接访问共享内存',
      hostApi: 'API: zeMemAllocHost() — CPU 可见，GPU cache coherent',
      deviceApi: 'API: zeMemAllocDevice() — GPU 本地内存 (iGPU 仍是系统内存，但标记为 GPU 优先)',
      sharedApi: 'API: zeMemAllocShared() — 统一地址空间，迁移引擎自动优化页面位置',
    },
    en: {
      title: 'iGPU Unified Memory Model: Host / Device / Shared (USM)',
      cpuLabel: 'CPU',
      cpuModel: 'Core i7-1370P',
      cpuCores: '14 cores',
      igpuLabel: 'iGPU',
      igpuModel: 'Xe2-LPG',
      igpuCores: '8 Xe cores',
      systemMemory: 'Unified System Memory (LPDDR5x)',
      systemMemoryDesc: 'CPU and iGPU share physical memory, no dedicated VRAM',
      fast: 'fast',
      cacheCoherent: 'cache coherent',
      slow: 'slow',
      optimized: 'optimized',
      direct: 'direct',
      zeroCopy: 'Zero-copy!',
      hostDesc: 'Host: CPU-optimized, GPU accesses via cache coherence (good for CPU-heavy workloads)',
      deviceDesc: 'Device: GPU-optimized, CPU access slow (good for GPU-heavy compute, sparse results)',
      sharedDesc: 'Shared (USM): Zero-copy! iGPU unique advantage, CPU/GPU direct access to shared memory',
      hostApi: 'API: zeMemAllocHost() — CPU visible, GPU cache coherent',
      deviceApi: 'API: zeMemAllocDevice() — GPU local memory (iGPU still system memory, but marked GPU-preferred)',
      sharedApi: 'API: zeMemAllocShared() — Unified address space, migration engine auto-optimizes page placement',
    },
  }[locale];
    ...
    <div className="my-6 p-4 border rounded-lg">
      <svg viewBox="0 0 580 340" className="w-full">
        ...
        <text x="290" y="305" fontFamily={FONTS.sans} fontSize="10" fill={COLORS.dark} textAnchor="middle" fontWeight="600">
          {mode === 'host' && t.hostDesc}
          {mode === 'device' && t.deviceDesc}
          {mode === 'shared' && t.sharedDesc}
        </text>
        <text x="290" y="320" fontFamily={FONTS.sans} fontSize="9" fill={COLORS.mid} textAnchor="middle">
          {mode === 'host' && t.hostApi}
          {mode === 'device' && t.deviceApi}
          {mode === 'shared' && t.sharedApi}
        </text>
      </svg>
    </div>
  );
