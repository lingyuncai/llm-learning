...
const GraphOptPasses: React.FC<{ locale?: 'zh' | 'en' }> = ({ locale = 'zh' }) => {
  const t = {
    zh: {
      step1Title: '原始计算图',
      step1Nodes: '个算子节点',
      step2Title: '常量折叠',
      step2Nodes: '个节点（BatchNorm 常量被折叠到 Conv 权重）',
      step3Title: '死节点消除',
      step3Nodes: '个节点（未使用的分支已移除）',
      step4Title: '算子融合',
      step4Nodes: '个节点（Conv+BN+ReLU 融合，MatMul+Add 融合）',
      fusedWeights: 'fused weights',
      fusedPrimitive: 'Fused primitive',
      step5Title: 'Layout 插入',
      step5Reorder: 'Reorder 开销在首次推理时由 cache 摊销',
      reorder: 'Reorder',
    },
    en: {
      step1Title: 'Original Compute Graph',
      step1Nodes: 'operator nodes',
      step2Title: 'Constant Folding',
      step2Nodes: 'nodes (BatchNorm constant folded into Conv weights)',
      step3Title: 'Dead Node Elimination',
      step3Nodes: 'nodes (unused branches removed)',
      step4Title: 'Operator Fusion',
      step4Nodes: 'nodes (Conv+BN+ReLU fused, MatMul+Add fused)',
      fusedWeights: 'fused weights',
      fusedPrimitive: 'Fused primitive',
      step5Title: 'Layout Insertion',
      step5Reorder: 'Reorder overhead amortized by cache on first inference',
      reorder: 'Reorder',
    },
  }[locale];
    ...
    },
    {
      ...
      content: (
        <svg viewBox="0 0 580 160" className="w-full">
          ...
          <text x={140} y={92} textAnchor="middle" fontFamily={FONTS.mono} fontSize={9} fill={COLORS.mid}>({t.fusedWeights})</text>
    ...
    },
    {
      ...
      content: (
        <svg viewBox="0 0 580 160" className="w-full">
          ...
          <text x={210} y={92} textAnchor="middle" fontFamily={FONTS.mono} fontSize={9} fill={COLORS.mid}>{t.fusedPrimitive}</text>
          ...
          <text x={380} y={92} textAnchor="middle" fontFamily={FONTS.mono} fontSize={9} fill={COLORS.mid}>{t.fusedPrimitive}</text>
    ...
    },
    {
      ...
      content: (
        <svg viewBox="0 0 580 160" className="w-full">
          ...
          <text x={125} y={82} textAnchor="middle" fontFamily={FONTS.mono} fontSize={10} fill={COLORS.orange}>{t.reorder}</text>
          ...
          <text x={465} y={82} textAnchor="middle" fontFamily={FONTS.mono} fontSize={10} fill={COLORS.orange}>{t.reorder}</text>
