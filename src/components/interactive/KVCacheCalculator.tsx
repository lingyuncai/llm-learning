...
export default function KVCacheCalculator({ locale = 'zh' }: { locale?: 'zh' | 'en' }) {
  const t = {
    zh: {
      modelPreset: '模型预设',
      seqLength: '序列长度 S',
      precision: '精度',
      concurrentReq: '并发请求 N',
      singleReq: '单请求 KV Cache',
      totalOccupancy: '个并发总占用',
      gpuMemory: '占 GPU 显存',
      totalMemory: '总显存',
      formula: '公式',
    },
    en: {
      ...
      seqLength: 'Sequence Length S',
      precision: 'Precision',
      concurrentReq: 'Concurrent Requests N',
      singleReq: 'Single Request KV Cache',
  ...
  return (
    <div className="my-6 p-4 border rounded-lg">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
        ...
        <div>
          <label className="text-xs text-gray-500 block">{t.seqLength}: {seqLen.toLocaleString()}</label>
        ...
        <div>
          <label className="text-xs text-gray-500 block">{t.concurrentReq}: {concurrency}</label>
      ...
      <div className="grid grid-cols-3 gap-4 mb-3">
        <div className="text-center p-3 bg-blue-50 rounded">
          <div className="text-xs text-gray-500">{t.singleReq}</div>
          <div className="text-lg font-bold text-blue-700">{result.singleGB.toFixed(3)} GB</div>
        </div>
        <div className="text-center p-3 bg-orange-50 rounded">
          <div className="text-xs text-gray-500">{concurrency} {t.totalOccupancy}</div>
        ...
        <div className="text-center p-3 rounded" style={{
          ...
          <div className="text-xs text-gray-500">{t.gpuMemory}</div>
      ...
      <div className="text-xs text-gray-400 mt-1 text-center">
        {hw.name}: {hw.memoryGB} GB {t.totalMemory}
      </div>

      <div className="mt-3 p-2 bg-gray-50 rounded text-xs text-gray-600 font-mono">
        {t.formula}: 2 × L({model.layers}) × kv_heads({model.kvHeads}) × S({seqLen.toLocaleString()}) × d_k({model.headDim}) × {bpp}B
      </div>
    </div>
  );
}
