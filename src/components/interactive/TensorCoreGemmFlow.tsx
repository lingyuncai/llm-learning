...
export default function TensorCoreGemmFlow({ locale = 'zh' }: { locale?: 'zh' | 'en' }) {
  const t = {
    zh: {
      step1Title: 'Step 1: load_matrix_sync — 加载 Fragment',
      step1Main: 'WMMA Step 1: 从 Shared Memory 加载 Fragment',
      step1Sub: '32 个线程协作将矩阵块加载到各自的寄存器中 (fragment)',
      sharedMemory: 'Shared Memory',
      warpThreadRegs: 'Warp (32 threads) — 各线程的寄存器',
      aFragment: 'A fragment',
      bFragment: 'B fragment',
      cAccumulator: 'C accumulator',
      codeComment1: '// 声明 fragment (每个线程持有矩阵的一部分)',
      fragmentDist: 'Fragment 分布',
      fragmentDetail: '16x16 = 256 个元素由 32 个线程分持: 每个线程寄存器中持有 8 个元素',
      fragmentOpaque: '具体哪个线程持有哪些元素由硬件决定 — 对程序员不透明 (只能通过 wmma API 操作)',
      step2Title: 'Step 2: mma_sync — Tensor Core 执行矩阵乘加',
      step2Main: 'WMMA Step 2: mma_sync — D = A * B + C',
      step2Sub: '一条 warp 级指令，32 个线程同步发射到 Tensor Core',
      tensorCore: 'Tensor Core',
      dAccumulatorUpdated: 'D accumulator (更新后)',
      inRegisters: '在寄存器中',
      perfNote: '16x16x16 = 4096 FMA (8192 FLOPs) / 指令 vs CUDA Core 1 FMA / 指令',
      step3Title: 'Step 3: store_matrix_sync — 写回结果',
      step3Main: 'WMMA Step 3: 累加完所有 tile 后写回',
      step3Sub: '内循环结束后，将 accumulator fragment 存回 shared memory 或 global memory',
      completeWarpLoop: '完整 Warp 级循环',
      dFragAccum: 'd_frag 在每个 k 步累加: d = a*b + d',
      globalMemC: 'Global Memory (C 矩阵)',
      wmmaThreeSteps: 'WMMA 三步: load_matrix_sync → mma_sync (循环) → store_matrix_sync',
      stillNeedBlockTiling: '仍然需要 Block 级 tiling (shared memory) — Tensor Core 只是替换了最内层计算单元',
    },
    en: {
      step1Title: 'Step 1: load_matrix_sync — Load Fragment',
      step1Main: 'WMMA Step 1: Load Fragment from Shared Memory',
      step1Sub: '32 threads cooperatively load matrix block into their registers (fragment)',
      sharedMemory: 'Shared Memory',
      warpThreadRegs: 'Warp (32 threads) — Thread Registers',
      aFragment: 'A fragment',
      bFragment: 'B fragment',
      cAccumulator: 'C accumulator',
      codeComment1: '// Declare fragment (each thread holds part of matrix)',
      fragmentDist: 'Fragment Distribution',
      fragmentDetail: '16x16 = 256 elements held by 32 threads: each thread holds 8 elements in registers',
      fragmentOpaque: 'Hardware determines which thread holds which elements — opaque to programmer (only accessible via wmma API)',
      step2Title: 'Step 2: mma_sync — Tensor Core Matrix Multiply-Add',
      step2Main: 'WMMA Step 2: mma_sync — D = A * B + C',
      step2Sub: 'One warp-level instruction, 32 threads synchronously dispatch to Tensor Core',
      tensorCore: 'Tensor Core',
      dAccumulatorUpdated: 'D accumulator (updated)',
      inRegisters: 'in registers',
      perfNote: '16x16x16 = 4096 FMA (8192 FLOPs) / instruction vs CUDA Core 1 FMA / instruction',
      step3Title: 'Step 3: store_matrix_sync — Write Back Result',
      step3Main: 'WMMA Step 3: Write back after accumulating all tiles',
      step3Sub: 'After inner loop completes, store accumulator fragment back to shared/global memory',
      completeWarpLoop: 'Complete Warp-level Loop',
      dFragAccum: 'd_frag accumulates at each k step: d = a*b + d',
      globalMemC: 'Global Memory (C matrix)',
      wmmaThreeSteps: 'WMMA Three Steps: load_matrix_sync → mma_sync (loop) → store_matrix_sync',
      stillNeedBlockTiling: 'Still need Block-level tiling (shared memory) — Tensor Core only replaces innermost compute unit',
    },
  }[locale];
      ...
      content: (
        <StepSvg>
          ...
          <rect x={30} y={55} width={120} height={60} rx={4}
            fill="#fff7ed" stroke={COLORS.orange} strokeWidth={1.5} />
          <text x={90} y={75} textAnchor="middle" fontSize="8" fontWeight="600"
            fill={COLORS.orange} fontFamily={FONTS.sans}>{t.sharedMemory}</text>
          <text x={90} y={92} textAnchor="middle" fontSize="7" fill="#64748b"
            fontFamily={FONTS.mono}>As[16x16] + Bs[16x16]</text>
          ...
          <rect x={215} y={50} width={340} height={80} rx={5}
            fill="#f8fafc" stroke="#cbd5e1" strokeWidth={1} />
          <text x={385} y={66} textAnchor="middle" fontSize="8" fontWeight="600"
            fill={COLORS.dark} fontFamily={FONTS.sans}>{t.warpThreadRegs}</text>
          <ThreadRegs x={225} y={74} w={100} h={44}
            label={t.aFragment} color={COLORS.primary} bg="#dbeafe"
            detail="wmma::matrix_a<16,16,16,half>" />
          <ThreadRegs x={340} y={74} w={100} h={44}
            label={t.bFragment} color={COLORS.green} bg="#dcfce7"
            detail="wmma::matrix_b<16,16,16,half>" />
          <ThreadRegs x={455} y={74} w={90} h={44}
            label={t.cAccumulator} color={COLORS.orange} bg="#fff7ed"
            detail="wmma::accumulator<float>" />
          ...
          <rect x={30} y={225} width={520} height={70} rx={5}
            fill="#dbeafe" stroke={COLORS.primary} strokeWidth={1} />
          <text x={W / 2} y={245} textAnchor="middle" fontSize="9" fontWeight="600"
            fill={COLORS.primary} fontFamily={FONTS.sans}>{t.fragmentDist}</text>
          <text x={60} y={265} fontSize="8" fill={COLORS.dark} fontFamily={FONTS.sans}>
            {t.fragmentDetail}
          </text>
          <text x={60} y={282} fontSize="8" fill="#64748b" fontFamily={FONTS.sans}>
    ...
    },
    {
      ...
      content: (
        <StepSvg>
          ...
          <ThreadRegs x={220} y={200} w={140} h={45}
            label={t.dAccumulatorUpdated} color={COLORS.purple} bg="#f3e8ff"
            detail={`FP32 (16x16) — ${t.inRegisters}`} />

          {/* Code */}
          <rect x={30} y={260} width={520} height={30} rx={4}
            fill="#1e293b" />
          <text x={45} y={279} fontSize="7.5" fill="#e2e8f0" fontFamily={FONTS.mono}>
            wmma::mma_sync(d_frag, a_frag, b_frag, c_frag);  // {locale === 'zh' ? '一条 warp 级指令完成 16x16x16 乘加' : 'one warp-level instruction completes 16x16x16 multiply-add'}
          </text>
      ...
      content: (
        <StepSvg>
          ...
          <rect x={30} y={55} width={520} height={100} rx={5}
            fill="#f8fafc" stroke="#e2e8f0" strokeWidth={1} />
          <text x={W / 2} y={72} textAnchor="middle" fontSize="9" fontWeight="600"
            fill={COLORS.dark} fontFamily={FONTS.sans}>{t.completeWarpLoop}</text>
          ...
          <rect x={200} y={220} width={180} height={30} rx={4}
            fill="#fef3c7" stroke={COLORS.orange} strokeWidth={1} />
          <text x={290} y={238} textAnchor="middle" fontSize="8" fontWeight="600"
            fill={COLORS.orange} fontFamily={FONTS.sans}>{t.globalMemC}</text>
