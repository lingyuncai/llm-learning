...
export default function MoAHierarchy({ locale = 'zh' }: { locale?: 'zh' | 'en' }) {
  const [arch, setArch] = useState<Arch>('flat');

  const t = {
    zh: {
      title: '层级 MoA 架构',
      flatLabel: 'HieraMAS 扁平',
      pyramidLabel: 'Pyramid MoA',
      layer: '层',
      flatDesc: 'HieraMAS: 节点内 LLM 混合 + 节点间通信',
      pyramidWide: '广',
      pyramidMid: '中',
      pyramidNarrow: '窄',
      pyramidDesc: 'Pyramid MoA: 层级递减 + decision-theoretic router 决定何时停止',
      comparisonTitle: '层级 MoA vs 扁平 MoA',
      flatMoA: '扁平 MoA: 所有模型同一层并行 → 综合。简单但质量收益有限。',
      hierMoA: '层级 MoA: 多层逐级精炼。每层综合后传给下一层进一步提升。',
      pyramidFeature: 'Pyramid MoA 特色: 层级递减（5→3→1），router 决定何时已经"够好了"可以提前终止。',
    },
    en: {
      title: 'Hierarchical MoA Architecture',
      flatLabel: 'HieraMAS Flat',
      pyramidLabel: 'Pyramid MoA',
      layer: '{t.layer}',
      flatDesc: 'HieraMAS: Intra-node LLM mixing + Inter-node communication',
      pyramidWide: 'Wide',
      pyramidMid: 'Mid',
      pyramidNarrow: 'Narrow',
      pyramidDesc: 'Pyramid MoA: Decreasing layers + decision-theoretic router for early stopping',
      comparisonTitle: 'Hierarchical MoA vs Flat MoA',
      flatMoA: 'Flat MoA: All models parallel in one layer → aggregate. Simple but limited quality gain.',
      hierMoA: 'Hierarchical MoA: Multi-layer progressive refinement. Each layer aggregates and passes to next for further improvement.',
      pyramidFeature: 'Pyramid MoA: Decreasing layers (5→3→1), router decides when "good enough" for early termination.',
    },
  }[locale];
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          {(['flat', 'pyramid'] as Arch[]).map((a, i) => (
            <g key={a}>
              ...
              <text x={i * 120 + 55} y="17" textAnchor="middle"
                ...
                {a === 'flat' ? t.flatLabel : t.pyramidLabel}
              </text>
            </g>
          ))}
        </g>
        ...
        {arch === 'flat' ? (
          <g transform="translate(30, 80)">
            {/* {t.layer} 1: 4 models */}
            <text x="0" y="10" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 1</text>
            ...
            {/* {t.layer} 2: 2 aggregators */}
            <text x="0" y="65" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 2</text>
            {['Agg-1', 'Agg-2'].map((m, i) => (
            ...
            {/* {t.layer} 3: Final aggregator */}
            <text x="0" y="120" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 3</text>
          ...
          <g transform="translate(30, 80)">
            ...
            <text x="0" y="10" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 1 ({t.pyramidWide})</text>
            ...
            <text x="0" y="55" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 2 ({t.pyramidMid})</text>
            ...
            <text x="0" y="100" fontFamily={FONTS.sans} fontSize="10" fontWeight="600" fill={COLORS.mid}>{t.layer} 3 ({t.pyramidNarrow})</text>
