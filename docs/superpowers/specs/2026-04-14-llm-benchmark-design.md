...
### 关键工具简介: lm-evaluation-harness

**注意**: 本节仅简要介绍 lm-eval-harness 的定位和重要性。框架架构、使用工作流、后端集成等细节在 Art.6 深入展开。

- **定位**: EleutherAI 维护的开源评估框架，Open LLM Leaderboard 的底层引擎
- **重要性**: 几乎所有开源模型的公开评估分数都经过它产生。理解这个工具就理解了大多数公开评估分数的生产方式
- **核心概念**: Task 定义（YAML 配置）、两种评估模式（loglikelihood 多选 / generate_until 生成）、标准化的结果输出
- **关键认知**: 同一个 benchmark 在不同 prompt 模板下分数可差 5-10%，这是分数不可比的主要原因之一
...
- BBH (Big-Bench Hard): 从 BIG-Bench 筛选的 23 个困难推理任务，测试多步推理和指令遵循
...
**时效性声明**: 本篇内容基于截至发布日期的公开技术报告。模型发布节奏快，标配集持续演进，建议定期更新。
...
各家小模型的评估策略（基于公开发布信息）:
- **Gemma 系列 (Google)**: 多语言评估、指令遵循（IFEval — Instruction Following Evaluation，测模型是否严格遵循格式/约束指令，小模型常报）、效率指标 [执行时需查阅最新 Gemma 4 发布]
...
Art.1-4 各有一个"流程动画"类组件（EvalProtocolFlow、MMLUProEvalDemo、SWEbenchFlow、BFCLEvalFlow），需要不同的视觉风格避免审美疲劳:
- **EvalProtocolFlow** (Art.1): 水平 pipeline 风格，tab 切换协议类型，偏抽象/通用
- **MMLUProEvalDemo** (Art.2): 卡片式题目展示，上方题目+选项，下方对比 direct vs CoT 两条路径
- **SWEbenchFlow** (Art.3): 垂直瀑布流/时间线风格，5 步从上到下，每步展开显示详情
- **BFCLEvalFlow** (Art.4): 左右对比面板（输入→输出），tab 切换不同调用类别
